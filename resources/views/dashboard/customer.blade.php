...
.cd-hero-date { font-size: .8rem; opacity: .7; font-weight: 500; }

/* ─── Stats cards ───────────────────────────────────────── */
.cd-stats-row { position: relative; z-index: 10; }
...
.cd-stat-icon.green   { background: #d1fae5; color: var(--cd-success); }
...
.cd-upcoming-label { font-size: .72rem; text-transform: uppercase; letter-spacing: 1px; color: #2563eb; font-weight: 700; }
.cd-upcoming-room  { font-size: 1.5rem; font-weight: 800; color: #1e3a8a; }
.cd-upcoming-meta  { font-size: .85rem; color: #3b82f6; }
.cd-upcoming-badge {
...
.cd-booking-card:hover { box-shadow: 0 6px 24px rgba(79,70,229,.1); transform: translateY(-2px); }
...
.cd-progress { height: 6px; border-radius: 999px; background: #f3f4f6; overflow: hidden; }
...
.cd-filter-tab button.active, .cd-filter-tab button:hover {
    background: #fff;
...
.cd-timeline li:last-child { border-bottom: none; }
.cd-tl-dot {
    width: 10px; height: 10px;
    border-radius: 50%; flex-shrink: 0; margin-top: 5px;
}
.cd-tl-dot.paid    { background: var(--cd-success); }
.cd-tl-dot.pending { background: var(--cd-warning); }
.cd-tl-dot.reject  { background: var(--cd-danger); }
...
.cd-modal .modal-header  { background: linear-gradient(135deg, var(--cd-primary), #7c3aed); color: #fff; border-radius: var(--cd-radius) var(--cd-radius) 0 0; border: none; }
.cd-modal .modal-header .btn-close { filter: invert(1); }
.cd-modal .modal-footer  { border-top: 1px solid var(--cd-border); }
.cd-modal .form-control, .cd-modal .form-select {
    border-radius: 10px; border-color: var(--cd-border);
...
.cd-modal .form-control:focus, .cd-modal .form-select:focus {
    border-color: var(--cd-primary); box-shadow: 0 0 0 3px rgba(79,70,229,.15);
}
.cd-drop-zone {
...
.cd-drop-zone:hover, .cd-drop-zone.drag-over { border-color: var(--cd-primary); background: #ede9fe; }
.cd-drop-zone input[type=file] { position: absolute; inset: 0; opacity: 0; cursor: pointer; }
...
.cd-animate { animation: slideUp .4s ease both; }
.cd-animate-d1 { animation-delay: .05s; }
.cd-animate-d2 { animation-delay: .1s;  }
.cd-animate-d3 { animation-delay: .15s; }
.cd-animate-d4 { animation-delay: .2s;  }
.cd-animate-d5 { animation-delay: .25s; }
</style>
@endsection
...
@php
    ...
    $avatarUrl = (auth()->user() && method_exists(auth()->user(), 'getAvatar'))
                 ? auth()->user()->getAvatar()
...
<div class="container">
    ...
    <div class="cd-hero cd-animate mb-4">
        ...
        <div class="d-flex align-items-center gap-3 flex-wrap mt-1" style="background:rgba(255,255,255,.12); border-radius:10px; padding:.75rem 1rem;">
            ...
            <div>
                ...
                <div style="font-size:1rem; font-weight:700;">Room {{ $ub->room->number }} &bull; {{ \Carbon\Carbon::parse($ub->check_in)->format('M d, Y') }}</div>
            </div>
            <span class="ms-auto" style="background:rgba(255,255,255,.25); border-radius:999px; padding:.3rem 1rem; font-size:.78rem; font-weight:700;">
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
                    ...
                    <div class="row g-2">
                        <div class="col-md-4">
                            <div class="cd-step">
                                ...
                                <div>
                                    <div class="cd-step-title"><i class="fas fa-university me-1 text-primary"></i> Pay</div>
                                    <div class="cd-step-desc">Transfer the total amount to one of the payment accounts listed below.</div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="cd-step">
                                ...
                                <div>
                                    <div class="cd-step-title"><i class="fas fa-upload me-1 text-primary"></i> Upload Receipt</div>
                                    <div class="cd-step-desc">Click <strong>"Pay / Upload Receipt"</strong> on your booking card and attach proof.</div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="cd-step">
                                ...
                                <div>
                                    <div class="cd-step-title"><i class="fas fa-shield-alt me-1 text-primary"></i> Get Confirmed</div>
                                    <div class="cd-step-desc">Our team will verify your payment and activate your booking shortly.</div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="cd-card cd-animate cd-animate-d2">
                ...
                <div class="cd-card-body">
                    ...
                    @php
                        $days      = \App\Helpers\Helper::getDateDifference($transaction->check_in, $transaction->check_out);
                    ...
                    @endphp
                    <div class="cd-booking-card mb-3"
                         data-status="{{ $transaction->status }}">
                        <div class="d-flex gap-3 align-items-start">
                            {{-- Room image --}}
                            <img src="{{ $roomImg }}" alt="Room {{ $transaction->room->number }}" class="room-img d-none d-sm-block">

                            {{-- Main info --}}
                            <div class="flex-grow-1 min-width-0">
                                <div class="d-flex align-items-center gap-2 flex-wrap mb-1">
                                    <span class="booking-id">#BK-{{ str_pad($transaction->id, 5, '0', STR_PAD_LEFT) }}</span>
                                    ...
                                    <span class="status-pill" style="background:#fee2e2;color:#991b1b;">
                                        <i class="fas fa-exclamation-triangle" style="font-size:.7rem;"></i> Urgent
                                    </span>
                                    @endif
                                </div>
                                ...
                                <div class="booking-room">Room {{ $transaction->room->number }}
                                    ...
                                    <span class="booking-type ms-1">· {{ $transaction->room->type->name }}</span>
                                    @endif
                                </div>

                                {{-- Dates + duration --}}
                                <div class="d-flex flex-wrap gap-3 mt-2 text-muted" style="font-size:.8rem;">
                                    <span><i class="fas fa-sign-in-alt me-1 text-success"></i>{{ \Carbon\Carbon::parse($transaction->check_in)->format('M d, Y') }}</span>
                                    <span><i class="fas fa-sign-out-alt me-1 text-danger"></i>{{ \Carbon\Carbon::parse($transaction->check_out)->format('M d, Y') }}</span>
                                    <span><i class="fas fa-moon me-1 text-primary"></i>{{ $days }} {{ \Illuminate\Support\Str::plural('Night', $days) }}</span>
                                </div>
                                ...
                                <div class="mt-3">
                                    <div class="d-flex justify-content-between align-items-center mb-1">
                                        ...
                                        <small class="fw-bold {{ $balance > 0 ? 'text-danger' : 'text-success' }}">
                                            {{ $paidPct }}% paid
                                        </small>
                                    </div>
                                    ...
                                    <div class="d-flex justify-content-between mt-1">
                                        <small class="text-muted">Paid: <strong>{{ \App\Helpers\Helper::convertToRupiah($paid) }}</strong></small>
                                        @if($balance > 0)
                                        <small class="text-danger">Balance: <strong>{{ \App\Helpers\Helper::convertToRupiah($balance) }}</strong></small>
                                        @else
                                        <small class="text-success"><i class="fas fa-check-circle me-1"></i>Fully Paid</small>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="d-flex align-items-center justify-content-between mt-3 flex-wrap gap-2">
                                    ...
                                    <div class="d-flex gap-2 flex-wrap">
                                        ...
                                        <form action="{{ route('transaction.cancel', $transaction->id) }}" method="POST" class="d-inline" id="cancelForm-{{ $transaction->id }}">
                                            @csrf
            ...
            <div class="cd-card mt-4 cd-animate cd-animate-d3">
                <div class="cd-card-header">
                    ...
                    <span class="badge bg-light text-muted">{{ $allPayments->count() }} {{ \Illuminate\Support\Str::plural('record', $allPayments->count()) }}</span>
                </div>
                <div class="cd-card-body p-3">
                    <ul class="cd-timeline">
                        ...
                        @endphp
                        <li>
                            <div class="cd-tl-dot {{ $dotClass }}" title="{{ $pmt->status }}"></div>
                            <div class="flex-grow-1">
                                <div class="d-flex justify-content-between flex-wrap gap-1">
                                    <div>
                                        ...
                                        <div style="font-size:.75rem; color:var(--cd-muted);">
                                            Booking #BK-{{ str_pad($pmt->transaction_id, 5, '0', STR_PAD_LEFT) }}
                                            @if($pmt->reference_number)
                                            · Ref: {{ $pmt->reference_number }}
                                            @endif
                                        </div>
                                    </div>
                                    <div style="font-size:.75rem; color:var(--cd-muted); white-space:nowrap;">
                                ...
                                <div class="mt-1">
                                    <a href="{{ asset($pmt->receipt_image) }}" target="_blank"
                                       class="text-decoration-none" style="font-size:.75rem; color:var(--cd-primary);">
        ...
        <div class="col-lg-4">
            ...
            {{-- Quick Actions --}}
            <div class="cd-card mb-4 cd-animate cd-animate-d1">
                <div class="cd-card-header"><h5><i class="fas fa-bolt text-warning me-2"></i>Quick Actions</h5></div>
                <div class="cd-card-body">
                    <div class="row g-2">
                        <div class="col-6">
                        ...
                        <div class="col-6">
                            <a href="{{ route('customer.show', auth()->user()->customer->id ?? 0) }}" class="cd-qa-btn">
            ...
            <div class="cd-card mb-4 cd-animate cd-animate-d2">
                ...
                <div class="cd-card-body d-flex flex-column gap-3">
                    ...
                    <div class="cd-bank-chip" onclick="copyAccNum('{{ $account->account_number }}')" title="Click to copy account number">
                        <div class="bank-name">
                    ...
                    @empty
                    <p class="text-muted small mb-0">No payment methods configured yet. Please contact support.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="cd-card cd-animate cd-animate-d3">
                ...
                <div class="cd-card-body">
                    <p class="text-muted small mb-3">Our front desk is happy to assist with bookings, payments, or any inquiries.</p>
                    ...
                    <div class="d-flex flex-column gap-2">
                        <a href="mailto:{{ $global_settings['contact_email'] ?? config('mail.from.address') }}"
                           ...
                           onmouseover="this.style.background='#f3f4f6'" onmouseout="this.style.background=''">
                            <div class="cd-stat-icon indigo" style="width:32px;height:32px;border-radius:8px;font-size:.9rem;">
                                <i class="fas fa-envelope"></i>
                            </div>
                            <span style="font-size:.85rem;">{{ $global_settings['contact_email'] ?? config('mail.from.address') }}</span>
                        </a>

                        @if(!empty($global_settings['contact_phone']))
                        <div class="d-flex align-items-center gap-2 p-2 rounded-2" style="color:#374151;">
                            <div class="cd-stat-icon green" style="width:32px;height:32px;border-radius:8px;font-size:.9rem;">
                        ...
                        <div class="d-flex align-items-center gap-2 p-2 rounded-2" style="color:#374151;">
                            <div class="cd-stat-icon amber" style="width:32px;height:32px;border-radius:8px;font-size:.9rem;">
                                <i class="fas fa-hotel"></i>
                            </div>
                            <span style="font-size:.85rem;">{{ $global_settings['hotel_name'] ?? 'Bella Vista Lodge' }} – Front Desk</span>
                        </div>
                    </div>
...
@if($transaction->status == 'Reservation')
@php $tp = $transaction->getTotalPrice(); $tpaid = $transaction->getTotalPayment(); $tbal = max(0,$tp-$tpaid); @endphp
<div class="modal fade cd-modal" id="payModal-{{ $transaction->id }}"
     tabindex="-1" aria-labelledby="payModalLabel-{{ $transaction->id }}" aria-hidden="true">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="payModalLabel-{{ $transaction->id }}">
                    ...
                    <small class="d-block fw-normal opacity-75" style="font-size:.78rem;">
                        Booking #BK-{{ str_pad($transaction->id, 5, '0', STR_PAD_LEFT) }} · Room {{ $transaction->room->number }}
                    </small>
                </h5>
            ...
            <form action="{{ route('transaction.uploadReceipt', $transaction->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    {{-- Summary strip --}}
                    <div class="d-flex justify-content-between p-3 mb-3 rounded-3" style="background:#f8fafc; font-size:.85rem;">
                        <div>
                    ...
                    {{-- Drop zone --}}
                    <div class="mb-3">
                        <label class="form-label fw-bold">Receipt / Proof of Payment <span class="text-danger">*</span></label>
                        <div class="cd-drop-zone" id="dropZone-{{ $transaction->id }}">
                            ...
                            <div id="dropHint-{{ $transaction->id }}">
                                ...
                                <div class="fw-bold" style="font-size:.9rem;">Drag & drop or click to browse</div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <span class="input-group-text" style="border-radius:10px 0 0 10px; font-size:.8rem;">TZS</span>
                    ...
                    <div class="mt-3 p-2 rounded-2 text-center" style="background:#fef3c7;">
                        <small class="text-warning fw-bold">
                            ...
                            After submission, our team will verify your payment within {{ $global_settings['receipt_verify_time'] ?? '1-2 hours' }}.
                        </small>
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content border-0" style="border-radius:16px; box-shadow:0 20px 60px rgba(0,0,0,.15);">
            <div class="modal-body text-center p-4">
                ...
                <p class="text-muted small mb-4">This action cannot be undone. Are you sure you want to cancel this booking?</p>
...
document.querySelectorAll('.cd-filter-tab button').forEach(btn => {
    btn.addEventListener('click', function () {
        document.querySelectorAll('.cd-filter-tab button').forEach(b => b.classList.remove('active'));
